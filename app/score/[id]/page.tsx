...
import {ArrowLeft, ArrowLeftCircle, ArrowRightCircle, Clock, Download, Eye, EyeOff, Fullscreen, Maximize2, Mic, Minimize2, Share2, SquareIcon, Star} from "lucide-react"
...
export default function ScorePage() {
    ...
    const {data: loadedScore, refetch} = useQuery({
        ...
        queryFn: async () => {
            ...
            if (fetchedDataRef.current) {
                console.log('Preventing duplicate score data fetch during development double-render');
    ...
    // Listen for page change events from ImageScoreRenderer
    useEffect(() => {
        const handlePageChange = (event: Event) => {
            const customEvent = event as CustomEvent;
            const { currentPage, scoreId } = customEvent.detail;
            
            if (scoreId === id || scoreId === score.file_id) {
                // Update the current page state when the user swipes in the renderer
    ...
    const ControlDock = () => {
        ...
        return (
            ...
            >
                ...
                {showRecordingsModal && (
                    <div className="absolute bottom-20 left-[calc(25%)] bg-gray-800/50 backdrop-blur-sm text-white rounded-lg shadow-lg p-4 w-64">
...
// Top bar with title and main controls
    ...
    useEffect(() => {
        ...
        const decodeNotes = async () => {
            try {
                const root = await protobuf.load('/static/notes.proto');
                const NoteList = root.lookupType('NoteList');
                const decoded = NoteList.decode(new Uint8Array(notesData));
                setNotes(decoded);
            } catch (error) {
        ...
        return (
            <div className="w-full h-screen overflow-hidden bg-gray-900">
                ...
                <div className="h-full w-full relative">
                    ...
                    {!showDock && (
                        <div className="fixed bottom-8 right-8 z-10 transition-opacity duration-300 opacity-80 hover:opacity-100">
    ...
    return (
        <Layout>
            <div className="relative h-[calc(100vh-5rem)]">
                ...
                {processedData && (
                    <div className="fixed top-[100px] right-[100px] bg-white dark:bg-gray-800/50 dark:backdrop-blur-sm p-4 rounded-lg shadow-lg">
                        <h3 className="text-lg font-bold">Processed Audio Data</h3>
                        <ul>
