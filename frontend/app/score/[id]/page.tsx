...
const DebugPanel = ({
  ...
  const [position, setPosition] = useState(JSON.parse(localStorage.getItem('debugPanelPosition') || '{"x": 20, "y": 100}'));
  ...
  const sendTestRequest = async (e?: React.MouseEvent) => {
    ...
    try {
      ...
      // WebM header and minimal scores structure
      ...
      // Concatenate the header and scores together
      ...
      // Try to decode the protobuf scores
      try {
  ...
  const renderTestStatus = () => {
    ...
    return (
      <div
        className={`text-xs mt-2 p-1 rounded ${testStatus.isError ? 'bg-red-900/50 text-red-200' : 'bg-green-900/50 text-green-200'}`}>
        {testStatus.message}
      </div>
    );
  };
